...
export async function Footer() {
  ...
  return (
    <footer className="grain relative bg-coal-950 text-cream-200">
      <div className="relative z-[2] mx-auto max-w-6xl px-5 pb-10 pt-24 sm:px-8 md:pt-28">
        ...
        */}
        <Reveal>
          <p className="font-display text-[clamp(2.75rem,1.5rem+6vw,6rem)] leading-none tracking-[-0.035em] text-cream-50">
            <Lines text={t.site.name} step={110} />
          </p>
          <p className="mt-5 max-w-md text-sm leading-relaxed text-taupe-300/75">
            {t.site.description}
          </p>
        </Reveal>
        ...
        <div className="mt-16 grid gap-10 border-t border-cream-50/10 pt-10 sm:grid-cols-2">
            ...
            <ul className="mt-5 space-y-2.5 text-sm">
              <li>
                <a
                  ...
                  dir="ltr"
                  className="inline-block transition-colors hover:text-cream-50"
                >
                  {CONTACT.phone}
                </a>
              </li>
              ...
              <li className="text-taupe-300/70">{t.footer.address}</li>
            </ul>
          </Reveal>
          ...
          <Reveal delay={160}>
            ...
            <ul className="mt-5 space-y-2.5 text-sm">
              {QUICK.map((link) => (
                <li key={link.href}>
                  <a href={link.href} className="transition-colors hover:text-cream-50">
                    {t.nav[link.key]}
                  </a>
                </li>
              ))}
      ...
      <div className="relative z-[2] border-t border-cream-50/10">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-5 py-6 text-xs text-taupe-300/60 sm:px-8">
          ...
          <p className="flex gap-6">
            <Link href="/privatnost" className="transition-colors hover:text-cream-50">
              {t.footer.privacy}
            </Link>
            <Link href="/uslovi" className="transition-colors hover:text-cream-50">
              {t.footer.terms}
            </Link>
          </p>
        </div>
